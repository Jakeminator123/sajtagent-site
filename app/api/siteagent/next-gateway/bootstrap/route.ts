...
export async function POST(request:Request) {
  ...
  try {
    ...
    // The fixed grant is 43 chars; bounded body prevents multipart/body DoS.
    const reader = request.body?.getReader()
    if (!reader) return new Response(null,{status:400,headers})
    let body="", length=0
    const decoder=new TextDecoder()
    while(true) { const {done,value}=await reader.read(); if(done)break; length+=value.length; if(length>512){await reader.cancel();return new Response(null,{status:413,headers})}; body+=decoder.decode(value,{stream:true}) }
    body+=decoder.decode()
    const result = await (await nextPreviewRepository()).exchangeGrant(new URLSearchParams(body).get("grant")??"",hostname)
