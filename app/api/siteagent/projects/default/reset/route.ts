...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!principal) {
    return errorResponse(401, "unauthenticated", "Logga in för att starta ett nytt projekt.")
  }
  ...
  try {
    ...
    const project = await new PostgresPersonalProjectRepositoryV1(pool)
      .resetPersonalStarterProject(principal)
    return Response.json({ schemaVersion: 1, ...project })
  } catch (error) {
    console.error(
      "[siteagent/projects/default/reset] project_reset_failed",
      safeErrorDiagnostic(error),
    )
    return errorResponse(503, "persistence_unavailable", "Utkastet kunde inte nollställas.")
  }
}
