...
`awaiting_user` instead of failing the conversation closed. The signed SSE
client accepts the same conversation prefix so recovery is not mock-only.
...
result; canonical acceptance adds `tool.completed`, `preview.ready`, and
`turn.completed:built`. A canned completion message is appended only when no
Runtime assistant text arrived. Site
