...
remote system passed. It targets the B project API and D `/next` routes. It does
...
build. Use test accounts, not a customer's working project.
...
- [ ] Run the live driver on a deployment with B/C/D integrated and record its reviewed Git SHAs and timestamp.
