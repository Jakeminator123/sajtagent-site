...
export const GENERIC_TURN_FAILED_MESSAGE_V1 =
  "Sajtagent kunde inte slutföra svaret. Försök igen eller omformulera frågan."
...
export const VISIBLE_ANSWER_UNCLOSED_MESSAGE_V1 =
  "Sajtagent visade ett svar, men turnen stängdes inte rent."
...
const SAFE_TURN_FAILED_BY_CODE_V1: Record<string, string> = {
  ...
  build_join_rejected: "Bygget kunde inte slutföras. Ingen preview accepterades.",
  openclaw_empty_answer: "Sajtagent slutförde turen utan ett visningsbart svar.",
...
export function publicTurnFailedMessageV1(input: {
  ...
  if (containsPrivateReasoningV1(input.message)) {
    return input.hasPublicMessage
      ? VISIBLE_ANSWER_UNCLOSED_MESSAGE_V1
      : PRIVATE_REASONING_BLOCKED_MESSAGE_V1
  }
  const known = SAFE_TURN_FAILED_BY_CODE_V1[input.code]
  if (known) {
    return input.hasPublicMessage ? VISIBLE_ANSWER_UNCLOSED_MESSAGE_V1 : known
  }
...
export function publicRuntimeCatchMessageV1(
  error: unknown,
  options: { hasPublicMessage?: boolean } = {},
): {
  ...
  if (options.hasPublicMessage) {
    return { code: "runtime_invalid", message: VISIBLE_ANSWER_UNCLOSED_MESSAGE_V1 }
  }
