...
// Utilities required by the original v0 IRL lanyard template.
...
export function encryptLanyardData(username: string, variant: CardVariant): string {
  if (!username) return ''
  const encoded = utf8ToBase64(`${OBFUSCATION_KEY}:${variant}:${username}`)
...
export function decryptLanyardData(encrypted: string): LanyardData | null {
  ...
  try {
    ...
    if (!decoded.startsWith(`${OBFUSCATION_KEY}:`)) return null
