...
check(
  ...
  "a visible answer plus private fail text keeps the reply and a safe closer",
)
check(
  publicTurnFailedMessageV1({
    code: "custom_safe",
...
check(
  publicRuntimeCatchMessageV1(
    new Error("runtime_message_contains_private_reasoning"),
    { hasPublicMessage: true },
  ).message === VISIBLE_ANSWER_UNCLOSED_MESSAGE_V1,
  "later private reasoning does not hide an already public reply",
)
check(
...
check(
  buildSuccessAssistantDeltaV1("Jag har byggt startsidan med hero.") === null,
  "existing Runtime summary is not replaced by a canned or status delta",
)
