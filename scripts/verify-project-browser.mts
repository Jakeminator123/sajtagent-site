...
assert.match(store, /const selectProject = useCallback\([\s\S]*?if \(resettingProjectRef.current \|\| abortRef.current\) return[\s\S]*?window.location.assign\(withLandingDraft/, "project switching must synchronously reject an in-flight turn before carrying its draft")
assert.match(store, /const newChat = useCallback\([\s\S]*?if \(resettingProjectRef.current \|\| abortRef.current\) return/, "new chat must not reset a turn awaiting acknowledgement")
assert.match(store, /const newProject = useCallback\([\s\S]*?if \(abortRef.current\)[\s\S]*?ok: false[\s\S]*?await createProject/, "new project must reject an in-flight turn before making its API request")
